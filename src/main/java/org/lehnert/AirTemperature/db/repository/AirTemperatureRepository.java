...
public interface AirTemperatureRepository extends CrudRepository<AirTemperature, Long> {
    @Query(value = "select * from AIR_TEMPERATURE where DATE between :first and :last Order by DATE",nativeQuery = true)
    Iterable<AirTemperature> getAirTemperatureByBetweenFirstAndLast(@Param("first") java.sql.Date first,
